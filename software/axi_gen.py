...
['`OUTPUT(', AXI_ADDR_W,   'axil_awaddr',  'Address write channel address'], \
...
['`OUTPUT(', AXI_DATA_W,   'axil_wdata',   'Write channel data'], \
['`OUTPUT(', AXI_DATA_W/8, 'axil_wstrb',   'Write channel write strobe'], \
...
['`OUTPUT(', AXI_ADDR_W,   'axil_araddr',  'Address read channel address'], \
...
['`INPUT(', AXI_DATA_W,    'axil_rdata',   'Read channel data'], \
...
def axi_m_port(prefix):
    for i in range(len(axi_m)):
        fout.write(axi_m[i][0]+prefix+axi_m[i][2]+', '+str(axi_m[i][1])+'), //'+axi_m[i][3]+'\n')
    
def axi_s_port(prefix):
    for i in range(len(axi_m)):
        fout.write(reverse(axi_m[i][0])+prefix+axi_m[i][2]+', '+str(axi_m[i][1])+'), //'+axi_m[i][3]+'\n')


def axi_portmap(prefix):
    for i in range(len(axi_m)):
        fout.write('.'+prefix+axi_m[i][2]+'('+prefix+axi_m[i][2]+', '+str(axi_m[i][1])+'), //'+axi_m[i][3]+'\n')

def axi_m_tb(prefix):
    for i in range(len(axi_m)):
        fout.write(tbsignal(axi_m[i][0])+prefix+axi_m[i][2]+', '+str(axi_m[i][1])+'), //'+axi_m[i][3]+'\n')
    
def axi_s_tb(prefix):
    for i in range(len(axi_m)):
        fout.write(tbsignal(reverse(axi_m[i][0]))+prefix+axi_m[i][2]+', '+str(axi_m[i][1])+'), //'+axi_m[i][3]+'\n')

def axi_wire(prefix):
    for i in range(len(axi_m)):
        fout.write('`WIRE('+prefix+axi_m[i][2]+', '+str(axi_m[i][1])+'), //'+axi_m[i][3]+'\n')


#AXI LITE

def axil_m_port(prefix):
    for i in range(len(axil_m)):
        fout.write(axil_m[i][0]+prefix+axil_m[i][2]+', '+str(axil_m[i][1])+'), //'+axil_m[i][3]+'\n')
    
def axil_s_port(prefix):
    for i in range(len(axil_m)):
        fout.write(reverse(axil_m[i][0])+prefix+axil_m[i][2]+', '+str(axil_m[i][1])+'), //'+axil_m[i][3]+'\n')


def axil_portmap(prefix):
    for i in range(len(axil_m)):
        fout.write('.'+prefix+axil_m[i][2]+'('+prefix+axil_m[i][2]+', '+str(axil_m[i][1])+'), //'+axil_m[i][3]+'\n')

def axil_m_tb(prefix):
    for i in range(len(axil_m)):
        fout.write(tbsignal(axil_m[i][0])+prefix+axil_m[i][2]+', '+str(axil_m[i][1])+'), //'+axil_m[i][3]+'\n')
    
def axil_s_tb(prefix):
    for i in range(len(axil_m)):
        fout.write(tbsignal(reverse(axil_m[i][0]))+prefix+axil_m[i][2]+', '+str(axil_m[i][1])+'), //'+axil_m[i][3]+'\n')

def axil_wire(prefix):
    for i in range(len(axil_m)):
        fout.write('`WIRE('+prefix+axil_m[i][2]+', '+str(axil_m[i][1])+'), //'+axil_m[i][3]+'\n')
...
if len(sys.argv) < 3 or len(sys.argv) > 5:
    ...
    quit()
if len(sys.argv) > 3:
    AXI_ADDR_W = sys.argv[3]
    AXIL_ADDR_W = sys.argv[3]
if len(sys.argv) > 4:
    AXI_DATA_W = sys.argv[4]
    AXIL_DATA_W = sys.argv[4]

prefix = sys.argv[1]+'_'
typ = sys.argv[2]

fout = open (prefix+typ+"_tab.tex", 'w')
