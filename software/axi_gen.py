...
def axi_m_port(fout):
    for i in range(len(axi_m)):
        ...
        fout.write(' '+axi_m[i][0]+axi_m[i][2]+', '+width+'), //'+axi_m[i][3]+'\n')
...
def axil_m_port(fout):
    for i in range(len(axil_m)):
        try: width = str(int(axil_m[i][1]));
        except: width = axil_m[i][1];
        fout.write(' '+axil_m[i][0]+axil_m[i][2]+', '+width+'), //'+axil_m[i][3]+'\n')
    
def axil_s_port(fout):
    for i in range(len(axil_m)):
        try: width = str(int(axil_m[i][1]));
        except: width = axil_m[i][1];
        fout.write(' '+reverse(axil_m[i][0])+axil_m[i][2]+', '+width+'), //'+axil_m[i][3]+'\n')

def axil_portmap(prefix, fout):
    for i in range(len(axil_m)):
        fout.write('.'+axil_m[i][2]+'('+prefix+axil_m[i][2]+'), //'+axil_m[i][3]+'\n')

def axil_m_tb(prefix, fout):
    for i in range(len(axil_m)):
        try: width = str(int(axil_m[i][1]));
        except: width = axil_m[i][1];
        fout.write(tbsignal(axil_m[i][0])+prefix+axil_m[i][2]+', '+width+') //'+axil_m[i][3]+'\n')
    
def axil_s_tb(prefix, fout):
    for i in range(len(axil_m)):
        fout.write(tbsignal(reverse(axil_m[i][0]))+prefix+axil_m[i][2]+', '+str(int(axil_m[i][1]))+') //'+axil_m[i][3]+'\n')

def axil_wire(prefix, fout):
    for i in range(len(axil_m)):
...
def main ():
    ...
    if len(sys.argv) > 3:
        try:
            ADDR_W = int(sys.argv[3])
        except:
            ADDR_W = sys.argv[3]

    if len(sys.argv) > 4:
        try:
            DATA_W = int(sys.argv[4])
        except:
            DATA_W = sys.argv[4]
    ...
    global axi_m
    global axil_m
    axi_m = make_axi(ADDR_W, DATA_W)
    axil_m = make_axil(ADDR_W, DATA_W)
